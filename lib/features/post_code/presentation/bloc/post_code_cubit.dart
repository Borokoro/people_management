...
class PostCodeCubit extends Cubit<PostCodeState> {
  ...
  PostCodeCubit({required this.getPostCodeDataUseCase}) : super(const PostCodeState());

  void getPostCodeData(String postCode) async{
    emit(const PostCodeLoading());
    final Either<Failure, PostCodeDataModel> result = await getPostCodeDataUseCase.call(postCode);
    result.fold((l) => emit(const PostCodeError()), (r) => emit(PostCodeLoaded(postCodeDataModel: r)));
  }
}
