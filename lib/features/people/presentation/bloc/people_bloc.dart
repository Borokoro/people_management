...
class PeopleBloc extends Bloc<PeopleEvent, PeopleState> {
  ...
  PeopleBloc({
    ...
    required this.insertPersonUseCase,
  ...
  _onDeletePersonEvent(
      DeletePersonEvent event, Emitter<PeopleState> emit) async {
    final Either<Failure, void> result =
        await deletePersonUseCase.call(event.id);
    result.fold((l) {}, (r) {
  ...
  _onGetPeopleEvent(GetPeopleEvent event, Emitter<PeopleState> emit) async {
    ...
    final Either<Failure, List<PeopleModel>> result =
        await getPeopleUseCase.call();
  ...
  _onInsertPersonEvent(
      InsertPersonEvent event, Emitter<PeopleState> emit) async {
    final Either<Failure, void> result =
        await insertPersonUseCase.call(event.person);
    result.fold((l) {}, (r) {
      event.context.read<PeopleBloc>().add(const GetPeopleEvent());
    });
  }
  ...
  _onUpdatePersonEvent(
      UpdatePersonEvent event, Emitter<PeopleState> emit) async {
    final Either<Failure, void> result =
        await updatePersonUseCase.call(event.person);
    result.fold((l) {}, (r) {
      event.context.read<PeopleBloc>().add(const GetPeopleEvent());
    });
  }
